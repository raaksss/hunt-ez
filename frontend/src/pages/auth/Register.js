...
const Register = () => {
  ...
  const{name,email,institnname,password,password2}=formData;
  const handleInputChange=(e)=>{
    ...
    const register=async (e)=>{
      e.preventDefault();
if(!name || !email || !institnname || !password)
...
const userData={
  name,email,institnname,password
};
setisLoading(true)
try {
...
<input type="text" placeholder='Institution Name' required name="institnname" value={institnname} onChange={handleInputChange}/>
